...
class Direction {
    constructor(directionCode) {
        this._direction = directionCode[0];
        this._distance = parseInt(directionCode.slice(1));
    }
...
function move(direction, currentCoord) {

    let newCoord;

    if (direction._direction === "U") {
        newCoord = new Coordinate(currentCoord._x, currentCoord._y + 1);
    } else if (direction._direction === "D") {
        newCoord = new Coordinate(currentCoord._x, currentCoord._y - 1);
    } else if (direction._direction === "L") {
        newCoord = new Coordinate(currentCoord._x - 1, currentCoord._y);
    } else if (direction._direction === "R") {
        newCoord = new Coordinate(currentCoord._x + 1, currentCoord._y);
    }
...
function calculatePath(directions) {
    let coordList = [];
    let currentCoord = new Coordinate(0, 0);

    for (const directionCode of directions) {

        let direction = new Direction(directionCode);

        while (direction._distance > 0) {
            let newCoord = move(direction, currentCoord);

            coordList.push(newCoord);
            currentCoord = newCoord;

            direction.decreaseDistance();
        }
    }
...
function findClosestIntersection(paths) {

    let closestIntersection = Number.MAX_SAFE_INTEGER;
    let zeroIndexOffset = 2; //arrays index from 0, so we need to add one for each index to calculate the length

    paths[0].forEach(function (coord, indexA) {
        let intersectionIndexB = paths[1].findIndex(c => {
            return c._x === coord._x && c._y === coord._y
        });
        if (intersectionIndexB > -1 && (indexA + intersectionIndexB) < closestIntersection) {
            closestIntersection = indexA + intersectionIndexB + zeroIndexOffset;
        }
    });
...
let paths = [];
paths.push(calculatePath(input[0]));
paths.push(calculatePath(input[1]));

document.querySelector("#app").innerHTML = findClosestIntersection(paths);
