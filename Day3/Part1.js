...
function calculateManhattanDistance(coord) {
    return Math.abs(coord._x) + Math.abs(coord._y);
}
...
function findClosestIntersection(paths) {
    ...
    for (const coord of paths[0]) {
        let intersection = paths[1].findIndex(c => {
            return c._x === coord._x && c._y === coord._y
        });
        if (intersection > -1 && calculateManhattanDistance(coord) < closestIntersection) {
            closestIntersection = calculateManhattanDistance(coord);
        }
    }
